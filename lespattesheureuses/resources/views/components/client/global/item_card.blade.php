...
<article class="flex flex-col relative gap-6 border-4 overflow-hidden bg-white border-primary rounded-3xl">
    ...
    <img src="{{$image_src}}" srcset="{{$image_src}} 300w,{{$image_src}} 600w,{{$image_src}} 900w,{{$image_src}} 1200w"
         sizes="(max-width: 768px) 100vw, 50vw" alt="{{$image_alt}}" class="border-primary">
    ...
    <div class="w-auto mx-auto {{$isAnimal ? 'mb-6' : ''}}">
        @if($isAnimal)
            <x-client.global.cta route="{{route('client_animal', ['animal' => $id])}}"
                                 title="{!! __('client/animals.see_me', ['name' => $name]) !!}">{!! __('global.see_me') !!}</x-client.global.cta>
        @endif
    </div>
</article>
