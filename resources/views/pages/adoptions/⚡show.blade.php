    ...
    <section class="relative">
        ...
        <div class="flex flex-col md:flex-row md:gap-30 md:items-center mb-8">
            ...
            <div class="md:w-1/2 aspect-square" wire:key="animal-image-{{$this->adoption->animal->id}}">
                <img src="{{Storage::disk('s3')->url('avatars/originals/'.$this->adoption->animal->avatar)}}"
                     srcset="
                        {{Storage::disk('s3')->url('avatars/variants/300x300/'.$this->adoption->animal->avatar)}} 300w,
                        {{Storage::disk('s3')->url('avatars/variants/600x600/'.$this->adoption->animal->avatar)}} 600w,
                        {{Storage::disk('s3')->url('avatars/variants/900x900/'.$this->adoption->animal->avatar)}} 900w"
